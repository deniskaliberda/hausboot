import { Text, Section } from "@react-email/components";
import { EmailLayout } from "../components/EmailLayout";

interface BookingInquiryAdminProps {
  inquiryNumber: string;
  name: string;
  email: string;
  phone: string;
  checkIn: string;
  checkOut: string;
  numGuests: number;
  message?: string;
}

export function BookingInquiryAdmin(props: BookingInquiryAdminProps) {
  return (
    <EmailLayout preview={`Neue Buchungsanfrage: ${props.inquiryNumber} – ${props.name}`}>
      <Text style={heading}>Neue Buchungsanfrage</Text>

      <Section style={card}>
        <Text style={label}>Anfragenummer</Text>
        <Text style={value}>{props.inquiryNumber}</Text>
        <Text style={label}>Name</Text>
        <Text style={value}>{props.name}</Text>
        <Text style={label}>E-Mail</Text>
        <Text style={value}>{props.email}</Text>
        <Text style={label}>Telefon</Text>
        <Text style={value}>{props.phone}</Text>
        <Text style={label}>Anreise</Text>
        <Text style={value}>{props.checkIn}</Text>
        <Text style={label}>Abreise</Text>
        <Text style={value}>{props.checkOut}</Text>
        <Text style={label}>Gäste</Text>
        <Text style={value}>{props.numGuests}</Text>
        {props.message && (
          <>
            <Text style={label}>Nachricht</Text>
            <Text style={value}>{props.message}</Text>
          </>
        )}
      </Section>
    </EmailLayout>
  );
}

const heading = { fontSize: "24px", fontWeight: "bold" as const, color: "#111827", fontFamily: "Georgia, serif", margin: "0 0 16px 0" };
const card = { backgroundColor: "#eff6ff", borderRadius: "8px", padding: "16px", border: "1px solid #bfdbfe" };
const label = { fontSize: "12px", fontWeight: "600" as const, color: "#6b7280", textTransform: "uppercase" as const, margin: "8px 0 2px 0" };
const value = { fontSize: "14px", color: "#111827", margin: "0" };

export default BookingInquiryAdmin;
